...
static void smb349_otg_status(enum usb_otg_state to, enum usb_otg_state from, void *data)
{
	...
	if ((from == OTG_STATE_A_SUSPEND) && (to == OTG_STATE_A_HOST)) {
		...
		/* Configure 1000mA to OTG */		
		ret = smb349_read(client, SMB349_OTG_TLIM_REG);		
		if (ret < 0) {			
			dev_err(&client->dev, "%s: err %d\n", __func__, ret);
		}		
		ret = smb349_write(client, SMB349_OTG_TLIM_REG, (ret | 0x0F));
		if (ret < 0) {			
			dev_err(&client->dev, "%s: err %d\n", __func__, ret);
		}
		/* Configure PGOOD to be active low */		
	...
	else if((from == OTG_STATE_B_PERIPHERAL) && (to == OTG_STATE_A_SUSPEND) && !charger->cable_exist)
	{
		...
		/* Configure maxiam input current as 500mA for other charger*/	
		ret = smb349_read(client, SMB349_STAT_TIME_CTRL);		
		if (ret < 0) {			
			dev_err(&client->dev, "%s: err %d\n", __func__, ret);
		}
		ret = smb349_write(client, SMB349_STAT_TIME_CTRL, (ret & (~(1<<4))));
		if (ret < 0) {
			dev_err(&client->dev, "%s: err %d\n", __func__, ret);
			goto error;
		}
		/* Configure maxiam input current as 500mA after cable remove */		
		ret = smb349_write(client, SMB349_CHARGE, 0x00);	
		if (ret < 0) {		
			dev_err(&client->dev, "%s: err %d\n", __func__, ret);
		}
		/* Disable volatile writes to registers */
...
static void smb349_irq_delay_worker_interrupt(struct work_struct *work)
{
	...
	val = smb349_read(chip->client, 0x3D);
	if(!chip->suspend)
	{
...
//CL2N+
//&*&*&*20120806_HC1, Need to check		
	ret = register_otg_callback(smb349_otg_status, charger);
	if (ret < 0)
		goto error;
//&*&*&*20120806_HC2, Need to check
//CL2N-
